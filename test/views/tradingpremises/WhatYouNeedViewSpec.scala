...
class WhatYouNeedViewSpec extends AmlsViewSpec with Matchers {
  ...
  "WhatYouNeedView" must {
    ...
    "contain the expected content elements with multiple services no MSB" in new ViewFixture {
      def view = what_you_need(confirmAddressCall, 1, Some(BusinessActivities(Set(AccountancyServices, HighValueDealing))), None)
    ...
    "contain the expected content elements with one service being MSB with one MSB activities" in new ViewFixture {
      def view = what_you_need(agentCall, 1, Some(BusinessActivities(Set(MoneyServiceBusiness))), Some(BusinessMatchingMsbServices(Set(TransmittingMoney))))
    ...
    "contain the expected content elements with one service being MSB with multiple MSB activities" in new ViewFixture {
      def view = what_you_need(agentCall, 1, Some(BusinessActivities(Set(MoneyServiceBusiness))), Some(BusinessMatchingMsbServices(Set(TransmittingMoney, CurrencyExchange))))
    ...
    "contain the expected content elements with multiple services one being MSB with multiple MSB activities" in new ViewFixture {
      def view = what_you_need(agentCall, 1, Some(BusinessActivities(Set(MoneyServiceBusiness, AccountancyServices))), Some(BusinessMatchingMsbServices(Set(TransmittingMoney, CurrencyExchange))))
