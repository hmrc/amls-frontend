...
class YourTradingPremisesViewSpec extends AmlsViewSpec with MustMatchers with TradingPremisesGenerator {
  ...
  "YourTradingPremisesView" must {
    "have correct title, headings displayed, no complete/incomplete headers displayed" in new ViewFixture {
      ...
      def view = your_trading_premises(false, NotCompleted, Seq.empty[(TradingPremises, Int)], Seq.empty[(TradingPremises, Int)])
    ...
    "have an incomplete/complete sections with addresses displayed and edit/remove links" in new ViewFixture {
      ...
      doc.getElementById("detail-edit-0").attr("href") must be(controllers.tradingpremises.routes.CheckYourAnswersController.get(1).url)
      ...
      doc.getElementById("detail-edit-1").attr("href") must be(controllers.tradingpremises.routes.CheckYourAnswersController.get(2).url)
