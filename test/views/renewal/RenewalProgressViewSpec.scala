...
class RenewalProgressViewSpec extends AmlsViewSpec with MustMatchers {
  ...
  "The renewal progress view" must {
    ...
    "enable the submit registration button when can submit and renewal section complete" in new ViewFixture {
      override def view = renewal_progress(TaskList(Seq.empty), businessName, serviceNames, canSubmit = true,
    ...
    "not have the submit registration button when cannot submit because renewal section not complete" in new ViewFixture {
      override def view = renewal_progress(TaskList(Seq.empty), businessName, serviceNames, canSubmit = false,
    ...
    "show ready to submit renewal when information are completed" in new ViewFixture {
      ...
      override def view = renewal_progress(TaskList(Seq.empty), businessName, serviceNames, false, true, readyForRenewal, true)
    ...
    "show submit renewal link and text when information are not completed yet" in new ViewFixture {
      ...
      override def view = renewal_progress(TaskList(Seq.empty), businessName, serviceNames, false, true, readyForRenewal, false)
    ...
    "show the Nominated officer box with correct title, name and link" in new ViewFixture {
      ...
      def view = renewal_progress(TaskList(Seq.empty), businessName, serviceNames, false, true, readyForRenewal, false,
