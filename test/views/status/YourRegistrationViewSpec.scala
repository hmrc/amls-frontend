...
class YourRegistrationViewSpec extends AmlsViewSpec with Matchers with AmlsReferenceNumberGenerator {
  ...
  "YourRegistrationView" must {
    ...
    "contain registration information for status NotCompleted" in new ViewFixture {
      ...
      doc.getElementById("incomplete-description").html()                           must be(
        "Your application to register with HMRC is incomplete. You have 28 days to complete your application from when you last saved your progress."
      )
      ...
      doc.getElementById("return-to-saved-application").attr("href")                must be(
    ...
    "contain correct content for status SubmissionWithdrawn" in new ViewFixture {
      ...
      doc.getElementById("application-withdrawn-description-1").text() must be(
        "You have withdrawn your application to register with HMRC."
      )
      doc.getElementById("application-withdrawn-description-2").text() must be(
        "If the Money Laundering Regulations apply to your business, you need to be registered with an appropriate supervisory body."
      )
      doc.getElementById("application-withdrawn-description-3").text() must be(
        "If you’re not registered and carry out activities covered by the Money Laundering Regulations, you may be subject to civil sanctions or criminal proceedings."
      )
    ...
    "contain correct content for status SubmissionDecisionRejected" in new ViewFixture {
      ...
      doc.getElementById("application-rejected-description-1").text() must be(
        "Your application to register with HMRC has been rejected."
      )
      doc.getElementById("application-rejected-description-2").text() must be(
        "Your business is not registered with HMRC. Your business must not carry out activities that are covered by the Money Laundering Regulations."
      )
      Option(doc.getElementById("update-information"))                must be(None)
    ...
    "contain correct content for status SubmissionDecisionRevoked" in new ViewFixture {
      ...
      doc.getElementById("application-revoked-description-2").text() must be(
        "Your business is not registered with HMRC. Your business must not carry out activities that are covered by the Money Laundering Regulations."
      )
      Option(doc.getElementById("update-information"))               must be(None)
    ...
    "contain correct content for status SubmissionDecisionExpired" in new ViewFixture {
      ...
      doc.getElementById("application-expired-description-2").text() must be(
        "Your business is not registered with HMRC under the Money Laundering Regulations."
      )
      Option(doc.getElementById("update-information"))               must be(None)
    ...
    "contain correct content for status DeRegistered" in new ViewFixture {
      ...
      doc.getElementById("application-deregistered-description-1").text() must be(
        "You have deregistered your business."
      )
      doc.getElementById("application-deregistered-description-2").text() must be(
        "If the Money Laundering Regulations apply to your business, you need to be registered with an appropriate supervisory body."
      )
      doc.getElementById("application-deregistered-description-3").text() must be(
        "If you’re not registered and carry out activities covered by the Money Laundering Regulations, you may be subject to civil sanctions or criminal proceedings."
      )
      Option(doc.getElementById("update-information"))                    must be(None)
      doc.getElementById("registration-status").html()                    must include(
        "Not supervised. Deregistered on " + DateHelper.formatDate(deregistrationDate.value) + "."
      )
      doc.getElementById("new.application.button").html()                 must include("Start a new application")
    ...
    "contain registration information for status SubmissionReady" in new ViewFixture {
      ...
      doc.getElementById("registration-info").html()                  must include(
        "Your application to register with HMRC is ready to submit. You have 28 days to submit your application from when you last saved your progress."
      )
    ...
    "contain correct content for renewal statuses" when {
      "renewal section is complete" in new ViewFixture {
        ...
        def view = your_registration(
          ...
          unreadNotifications = 10,
          registrationStatus =
            registration_status(Some(amlsRegistrationNumber), ReadyForRenewal(renewalDate), endDate = renewalDate),
          feeInformation = Some(fee_information(ReadyForRenewal(renewalDate)))
        )

        doc.getElementById("registration-info").html()                              must include(
          "You have completed your renewal but have not submitted it. If you do not submit your renewal, your registration with HMRC will expire."
        )
        doc.getElementById("go-to-about-your-business").html()                      must include(
          "Check business information and submit renewal"
        )
        doc.getElementById("go-to-about-your-business").nextElementSibling().html() must include("for business Name")
        doc.getElementById("go-to-about-your-business").attr("href")                must be(
          controllers.routes.RegistrationProgressController.get().url
        )
      }
      ...
      "renewal section is incomplete" in new ViewFixture {
        ...
        doc.getElementById("registration-info").html()                     must include(
          "You have started your renewal but have not completed it. If you do not submit your renewal, your registration with HMRC will expire."
        )
        ...
        doc.getElementById("continue-renewal").attr("href")                must be(
      ...
      "renewal section is not started (due)" in new ViewFixture {
        ...
        doc.getElementById("registration-info").html()                  must include(
          "Your registration with HMRC will expire on " + DateHelper.formatDate(renewalDate.get) + " unless you renew."
        )
        ...
        doc.getElementById("start-renewal").attr("href")                must be(
          controllers.renewal.routes.WhatYouNeedController.get.url
        )
      }
      ...
      "renewal is submitted" in new ViewFixture {
        ...
        def view = your_registration(
          ...
          unreadNotifications = 10,
          registrationStatus =
            registration_status(Some(amlsRegistrationNumber), RenewalSubmitted(renewalDate), endDate = renewalDate),
          feeInformation = Some(fee_information(RenewalSubmitted(renewalDate)))
        )

        doc.getElementById("renewal-pending-description-1").text()                                 must be(
          "You have applied to extend your supervision. HMRC will review your application after we have received payment."
        )
        doc.getElementById("renewal-pending-description-2").text()                                 must be(
          "We’ll email you when we have made a decision."
        )
    ...
    "contain your business information cell with right content" in new ViewFixture {
      ...
      yourBusinessCell
        .getElementsMatchingOwnText("Check or update your business information")
    ...
    "contain your registration status information cell with right content for status SubmissionReadyForReview" in new ViewFixture {
      ...
      def view = your_registration(
        ...
        registrationStatus = registration_status(
          status = SubmissionReadyForReview,
    ...
    "contain your registration status information cell with right content for status SubmissionDecisionApproved" in new ViewFixture {
      ...
      def view = your_registration(
        ...
        yourRegistrationInfo = Some(Html("some registration information")),
        registrationStatus = registration_status(
          status = SubmissionDecisionApproved,
          amlsRegNo = Some("XBML0987654345"),
    ...
    "contain your registration status information cell with right content for status SubmissionDecisionApproved if endDate is not available" in new ViewFixture {
      ...
      def view = your_registration(
        ...
        yourRegistrationInfo = Some(Html("some registration information")),
        registrationStatus =
          registration_status(status = SubmissionDecisionApproved, amlsRegNo = Some("XBML0987654345"), endDate = None),
        feeInformation = None
      )
    ...
    "contain your messages cell with right content" in new ViewFixture {
      ...
      messagesCell
        .getElementsMatchingOwnText("Check your messages")
    ...
    "contain your fees cell with right content for status SubmissionDecisionApproved" in new ViewFixture {
      ...
      feeCell.getElementsByClass("govuk-heading-s").first().html() must include("Fees")
      feeCell
        .getElementsMatchingOwnText("How to pay")
        .attr("href")                                              must be("how-to-pay")
    }
    ...
    "contain additional content elements" in new ViewFixture {
      ...
      doc
        .getElementsMatchingOwnText("Print this page")
    ...
    "contain withdraw application link for status SubmissionReadyForReview" in new ViewFixture {
      ...
      doc
        .getElementsMatchingOwnText("withdraw your application")
    ...
    "contain deregister link for status SubmissionDecisionApproved" in new ViewFixture {
      ...
      doc
        .getElementsMatchingOwnText("deregister your business")
