...
class PassportNumberValidatorSpec extends PlaySpec with MockitoSugar with OneServerPerSuite {
  ...
  val passportNumberLengths = Seq(9, 7, 4, 40) //NewUKPassportNumber, OldUKPassportNumber, MinNonUKPassportNumber, MaxNonUKPassportNumber

  "mandatory UK Passport Number" should {
    "return the passport number if uk format is correct" in {
      mandatoryPassportNumber("uk passport", "blank message", "invalid length", "invalid value", passportNumberLengths: _*)
    ...
    "return the passport number if non UK format is correct" in {
      mandatoryPassportNumber("non uk passport", "blank message", "invalid length", "invalid value", passportNumberLengths: _*)
    ...
    "respond appropriately if unbound" in {
      mandatoryPassportNumber("uk passport", "blank message", "invalid length", "invalid value", passportNumberLengths: _*).
        binder.unbind("", "A123456") mustBe Map("" -> "A123456")
    }
  }
