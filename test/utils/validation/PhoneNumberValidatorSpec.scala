...
class PhoneNumberValidatorSpec extends PlaySpec with MockitoSugar with OneServerPerSuite {
  ...
  "mandatoryPhoneNumber" should {
    ...
    "respond appropriately for invalid phone numbers " in {
      mandatoryPhoneNumber("blank", "length", "invalid").bind(Map("" -> "(0191) 6678 899"))
        .left.getOrElse(Nil).contains(FormError("", "invalid")) mustBe true // mustBe Right("(0191) 6678 899")
      mandatoryPhoneNumber("blank", "length", "invalid").bind(Map("" -> "(0191) 6678 899#4456"))
        .left.getOrElse(Nil).contains(FormError("", "invalid")) mustBe true //mustBe Right("(0191) 6678 899#4456")
      mandatoryPhoneNumber("blank", "length", "invalid").bind(Map("" -> "(0191) 6678 899*6"))
        .left.getOrElse(Nil).contains(FormError("", "invalid")) mustBe true //mustBe Right("(0191) 6678 899*6")
      mandatoryPhoneNumber("blank", "length", "invalid").bind(Map("" -> "(0191) 6678-899"))
        .left.getOrElse(Nil).contains(FormError("", "invalid")) mustBe true //mustBe Right("(0191) 6678-899")
      mandatoryPhoneNumber("blank", "length", "invalid").bind(Map("" -> "01912224455 ext 5544"))
