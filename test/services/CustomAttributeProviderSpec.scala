  ...
  "CustomAttributeProvider" must {
    ...
    "correctly parse markdown with heading and bullet list" in {
      ...
      Jsoup.parse(CustomAttributeProvider.commonMark(rawText)).html().replaceAll(" ","") must be(
        Jsoup.parse("""<h1 class="govuk-heading-l">Test Heading</h1><ul class="govuk-list govuk-list--bullet"><li>bullet 1</li><li>bullet 2</li><li>bullet 3</li></ul>""").html().replaceAll(" ",""))
    ...
    "correctly parse ETMP derived markdown" in {
      ...
      Jsoup.parse(CustomAttributeProvider.commonMark(etmpDerived)).body().children().first().outerHtml() must be("""<h1 class="govuk-heading-l">Text Paragraph Heading</h1>""")
      Jsoup.parse(CustomAttributeProvider.commonMark(etmpDerived)).getElementsByTag("ul").hasClass("govuk-list--bullet") must be(true)
