  ...
  "The AddressModifiedAuditEvent" must {
    "create the proper detail" when {
      "given a current address and an old address" in {
        val currentAddress = AuditAddress("Addr Line 1", Some("Addr Line 2"), "Line 3".some, "Spain", "AA1 1AA".some)
        val oldAddress = AuditAddress("Old addr Line 1", Some("Old addr Line 2"), "Old line 3".some, "France", "NE1 1ET".some)
      ...
      "given a current address and an old address without the optional fields" in {
        val currentAddress = AuditAddress("Addr Line 1", Some("Addr Line 2"), None, "Spain", None)
        val oldAddress = AuditAddress("Old addr Line 1", Some("Old addr Line 2"), None, "France", None)
