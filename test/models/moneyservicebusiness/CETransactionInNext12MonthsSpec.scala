    ...
    "Form Validation" must {
      ...
      "fail validation on missing field" in {
        ...
        CETransactionsInNext12Months.formRule.validate(map) must be(Invalid(
          Seq( Path \ "ceTransaction" -> Seq(ValidationError("error.required.msb.ce.transactions.in.12months")))))
      }
      ...
      "fail validation on invalid field" in {
        ...
        CETransactionsInNext12Months.formRule.validate(map) must be(Invalid(
          Seq( Path \ "ceTransaction" -> Seq(ValidationError("error.invalid.msb.ce.transactions.in.12months.number")))))
      }
      ...
      "fail validation on invalid field when it exceeds the max length" in {
        ...
        CETransactionsInNext12Months.formRule.validate(map) must be(Invalid(
          Seq( Path \ "ceTransaction" -> Seq(ValidationError("error.invalid.msb.ce.transactions.in.12months")))))
      }
    ...
    "Json Validation" must {
      ...
      "Successfully read/write Json data" in {
        ...
        CETransactionsInNext12Months.format.reads(CETransactionsInNext12Months.format.writes(
          CETransactionsInNext12Months("12345678963"))) must be(JsSuccess(CETransactionsInNext12Months("12345678963"), JsPath))
