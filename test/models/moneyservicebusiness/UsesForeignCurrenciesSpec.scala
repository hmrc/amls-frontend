  ...
  "UsesForeignCurrencies" must {
    ...
    "JSON validation" must {
      ...
      "successfully validate given values" in {
        val json =  Json.obj("usesForeignCurrencies" -> true)
      ...
      "successfully validate given values with option No" in {
        val json =  Json.obj("usesForeignCurrencies" -> false)
      ...
      "fail when on path is missing" in {
        Json.fromJson[UsesForeignCurrencies](Json.obj()) must
          be(JsError((JsPath \"usesForeignCurrencies") -> play.api.data.validation.ValidationError("error.path.missing")))
      }

      "fail when on invalid data" in {
        Json.fromJson[UsesForeignCurrencies](Json.obj("usesForeignCurrencies" -> "")) must
          be(JsError((JsPath \ "usesForeignCurrencies") -> play.api.data.validation.ValidationError("error.expected.jsboolean")))
      }

      "write valid data in using json write" in {
        Json.toJson[UsesForeignCurrencies](UsesForeignCurrenciesYes) must be (Json.obj("usesForeignCurrencies" -> true))
      }

      "write valid data in using json write with Option No" in {
        Json.toJson[UsesForeignCurrencies](UsesForeignCurrenciesNo) must be (Json.obj("usesForeignCurrencies" -> false))
      }
    }
  }
}
