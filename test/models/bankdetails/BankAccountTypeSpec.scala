...
class BankAccountTypeSpec extends PlaySpec with MockitoSugar {
  ...
  "BankAccountType" must {
    ...
    "successfully validate form Read" in {
      BankAccountType.formReads.validate(Map("bankAccountType" -> Seq("01"))) must be (Success(PersonalAccount))
      BankAccountType.formReads.validate(Map("bankAccountType" -> Seq("02"))) must be (Success(BelongsToBusiness))
      BankAccountType.formReads.validate(Map("bankAccountType" -> Seq("03"))) must be (Success(BelongsToOtherBusiness))
      BankAccountType.formReads.validate(Map("bankAccountType" -> Seq("04"))) must be (Success(NoBankAccount))
    ...
    "successfully write form data" in {
      BankAccountType.formWrites.writes(PersonalAccount) must be (Map("bankAccountType" -> Seq("01")))
      BankAccountType.formWrites.writes(BelongsToBusiness) must be (Map("bankAccountType" -> Seq("02")))
      BankAccountType.formWrites.writes(BelongsToOtherBusiness) must be (Map("bankAccountType" -> Seq("03")))
      BankAccountType.formWrites.writes(NoBankAccount) must be (Map("bankAccountType" -> Seq("04")))
    }
