...
class AccountSpec extends PlaySpec with MockitoSugar {
  ...
  "Account details form" must {
    "fail to validate" when {
      "no selection is made for an account type" in {
        ...
        Account.isUkFormRead.validate(urlFormEncoded) must be(Invalid(Seq(
      ...
      "a UK account is selected and an empty account number is provided" in {
        ...
        Account.ukFormRead.validate(urlFormEncoded) must be(Invalid(Seq(
      ...
      "a UK account is selected and the sort code given is empty" in {
        ...
        Account.ukFormRead.validate(urlFormEncoded) must be(Invalid(Seq(
          (Path \ "sortCode") -> Seq(ValidationError("error.invalid.bankdetails.sortcode.length")))))
      }
      "a UK account is selected and the value given has an incorrect pattern" in {
        ...
        Account.ukFormRead.validate(urlFormEncoded) must be(Invalid(Seq(
          (Path \ "sortCode") -> Seq(ValidationError("error.invalid.bankdetails.sortcode.characters")))))
      }
      "a non UK account is selected and both the IBAN and non UK account number are empty" in {
        ...
        Account.nonUkIbanRead.validate(urlFormEncoded) must be(Invalid(Seq(
          (Path \ "IBANNumber") -> Seq(ValidationError("error.required.bankdetails.iban")))))
      }
      ...
      "a non UK account is selected and the non UK account number is greater than the max length permitted" in {
        ...
        Account.nonUkAccountNumberFormRead.validate(urlFormEncoded) must be(Invalid(Seq(
          (Path \ "nonUKAccountNumber") -> Seq(ValidationError("error.invalid.bankdetails.account.length")))))
      }
  ...
  "For the Account" must {
    ...
    "Form Rule validation is successful for UKAccount1" in {
      ...
      Account.nonUkAccountNumberFormRead.validate(urlFormEncoded) must be(Valid(NonUKAccountNumber("123456789012345678901234567890ABCDefghij")))
    }
    ...
    "Form Rule validation is successful for UKAccount2" in {
      ...
      Account.nonUkIbanRead.validate(urlFormEncoded) must be(Valid(NonUKIBANNumber("123456789012345678901234567890ABCD")))
    }
    ...
    "Form Write is successful for UKAccount" in {
      ...
      Account.ukFormWrites.writes(ukAccount) must be(urlFormEncoded)
    }
    ...
    "Form Write is successful for NonUKAccount" in {
      ...
      Account.nonUkAccountNumberFormWrites.writes(ukAccount) must be(urlFormEncoded)
    }
    ...
    "Form Write is successful for NonUKAccount iban" in {
      ...
      Account.nonUkIbanWrites.writes(ukAccount) must be(urlFormEncoded)
    }
    ...
    "Form Write validation for IBAN Non UK Account" in {
      ...
      Account.nonUkIbanWrites.writes(nonUKIBANNumber) must be(urlFormEncoded)
    }
    ...
    "JSON Read is successful for Non UKAccount with Account Number" in {
      ...
      Account.nonUkAccountNoJsonReads.reads(jsObject) must be(JsSuccess(NonUKAccountNumber("12345"), JsPath \ "nonUKAccountNumber"))
    }
    ...
    "Form Write for Non UK Account Number" in {
      ...
      Account.nonUkAccountNumberFormWrites.writes(nonUKAccount) must be(urlFormEncoded)
    }
  }
  ...
  "For the BankAccount" must {
    ...
    "Form Write for Non UK Account" in {
      ...
      Account.nonUkAccountNumberFormWrites.writes(nonUKBankAccount) must be(urlFormEncoded)
    }
    ...
    "Form Write validation for IBAN Non UK Account" in {
      ...
      Account.nonUkIbanWrites.writes(nonUKBankAccount) must be(urlFormEncoded)
    }
  ...
  "nonUKBankAccountNumberType" must {
    ...
    "fail validation if Non UK Account is longer than the permissible length" in {
      Account.nonUKBankAccountNumberType.validate("12345678901234567890123456789012345678901234567890") must be(
        Invalid(Seq(Path -> Seq(ValidationError("error.invalid.bankdetails.account.length")))))
    }
  ...
  "sortCodeType" must {
    ...
    "fail validation when more than 6 digits are supplied without - " in {
      Account.sortCodeType.validate("87654321") must be(
        Invalid(Seq(Path -> Seq(ValidationError("error.invalid.bankdetails.sortcode.length")))))
    }

    "fail when 8 non digits are supplied with - " in {
      Account.sortCodeType.validate("ab-cd-ef") must be(
        Invalid(Seq(Path -> Seq(ValidationError("error.invalid.bankdetails.sortcode.characters")))))
    }
