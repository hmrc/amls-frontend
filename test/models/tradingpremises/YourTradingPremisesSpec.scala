  ...
  "YourTradingPremises" must {
    ...
    "return valid response when isResidential and startDate are empty" in {
      YourTradingPremises.formR.validate(Map("tradingName" -> Seq("foo"),
        ...
        "addressLine2" -> Seq("2"),
        "postcode" -> Seq("AA11 1AA")
      )) must be (Valid(YourTradingPremises("foo",Address("1",None,None,None,"AA11 1AA",None),None,None,None)))
    ...
    "fail validation" when {
      "given a future date" in {
        ...
        YourTradingPremises.formR.validate(data) must be(Valid(YourTradingPremises("foo",Address("1",None,None,None,"AA11 1AA",None),Some(true),Some(LocalDate.now().plusDays(1)),None)))
0
