...
class YourTradingPremisesSpec extends WordSpec with MustMatchers {
  ...
  "LocalDate serialisation" must {

    "JSON READ creates the Date Object" in {
    ...
    "JSON WRITE reads the LocalDate to JSON" in {
      val localDate: LocalDate = LocalDate.parse("2015-08-15")
      YourTradingPremises.jsonWriteLocalDate.writes(localDate) must be(Json.obj("startOfTradingDate" -> "2015-08-15"))
    }
