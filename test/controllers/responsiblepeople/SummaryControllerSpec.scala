  ...
  "Post" must {
    ...
    "redirect to 'Who is the business’s nominated officer?'" when {
      "'fromDeclaration flat set to true and status is pending'" in new Fixture {
        ...
        val rp1 = ResponsiblePeople(Some(PersonName("first", Some("middle"), "last", None, None)), None, None, None, None, Some(positions))
        val rp2 = ResponsiblePeople(Some(PersonName("first2", None, "middle2", None, None)), None, None, None, None, Some(positions))
