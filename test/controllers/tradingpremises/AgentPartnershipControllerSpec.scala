...
class AgentPartnershipControllerSpec extends PlaySpec with OneAppPerSuite with MockitoSugar with ScalaFutures {
  ...
  "AgentPartnershipController" must {
    ...
    "post with valid data in edit mode" in new Fixture {
      ...
      status(result) must be(SEE_OTHER)
      redirectLocation(result) must be(Some(routes.AgentPartnershipController.get(1,true).url))
