...
class WithdrawalConfirmationControllerSpec extends AmlsSpec with Injecting {
  ...
  trait TestFixture extends AuthorisedFixture {
    ...
    lazy val controller = new WithdrawalConfirmationController(
  ...
  "when entering to the page it displays confirmation page" in new TestFixture {
    ...
    when(dataCacheConnector.fetch[WithdrawalReason](any(), eqTo(WithdrawalReason.key))(any())).thenReturn(
      Future.successful(Some(WithdrawalReason.OutOfScope))
    )
    ...
    document.select("h1").text mustBe "Application withdrawn" withClue "ensure the page displays right content"
  }
}
