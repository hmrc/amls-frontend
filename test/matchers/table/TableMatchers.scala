...
trait TableMatchers {
  ...
  class TableHeaderMatcher(tableHeaders: Seq[HeadCell]) extends Matcher[Table] {
    override def apply(left: Table): MatchResult = {
      ...
      if (left.head.nonEmpty && tableHeaders.isEmpty) {
        MatchResult(false, s"Table contained the following headers: ${left.head.get.mkString} but empty header sequence provided", "N/A")
      }

      val nonMatchingHeadCells = tableHeaders.foldLeft(Seq.empty[HeadCell]) { (nonMatchingHeadCells: Seq[HeadCell], curr: HeadCell) =>
  ...
  class TableNoHeaderMatcher() extends Matcher[Table] {
    override def apply(left: Aliases.Table): MatchResult = {
      MatchResult(!left.head.exists(_.nonEmpty), "Table has no headers", s"Table has the following headers: ${left.head}")
    }
  }
