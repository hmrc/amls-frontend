...
class SupervisionEndFormProviderSpec extends DateBehaviours {
  ...
  "Supervision End Date form" must {
    ...
    "fail to bind" when {
      ...
      fields foreach { field =>
        s"$field is blank" in {
          ...
          forAll(datesBetween(minDate, maxDate)) { date =>
            ...
            result.errors.headOption shouldEqual Some(
              FormError(s"$formField.$field", messages("error.supervision.end.required.date.one"), Seq(field))
            )
          }
        }
        ...
        s"$field is in the incorrect format" in {
          ...
          result.errors.headOption shouldEqual Some(
            FormError(s"$formField.$field", messages("error.supervision.end.invalid.date.one"), Seq(field))
          )
        }
      }
      ...
      fieldsTwo foreach { fields =>
          ...
          forAll(datesBetween(minDate, maxDate)) { date =>
            ...
            result.errors.headOption shouldEqual Some(
              FormError(
                ...
                messages("error.supervision.end.required.date.two"),
        ...
        s"${fields._1} and ${fields._2} are in the incorrect format" in {
          ...
          result.errors.headOption shouldEqual Some(
            FormError(
              s"$formField.${fields._1}",
              messages("error.supervision.end.invalid.date.multiple"),
              Seq(fields._1, fields._2)
            )
          )
        }
    ...
    behave like dateFieldWithMin(
      ...
      formField,
      minDate,
      FormError(formField, "error.supervision.end.invalid.date.after.1900")
    )
    ...
    behave like dateFieldWithMax(
      ...
      formField,
      maxDate,
      FormError(formField, "error.supervision.end.invalid.date.future")
    )

    behave like realDateField(form, formField, "error.supervision.end.invalid.date.not.real")
  }
}
