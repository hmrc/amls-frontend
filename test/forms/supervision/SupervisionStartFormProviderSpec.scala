      ...
      fieldsTwo foreach { fields =>
        ...
        s"${fields._1} and ${fields._2} are blank" in {
          ...
          forAll(datesBetween(minDate, maxDate)) { date =>
            ...
            result.errors.headOption shouldEqual Some(
              FormError(s"$formField.${fields._1}", messages("error.supervision.start.required.date.two"), Seq(fields._1, fields._2))
            )
          }
        }
        ...
        s"${fields._1} and ${fields._2} are in the incorrect format" in {
          ...
          result.errors.headOption shouldEqual Some(
            FormError(s"$formField.${fields._1}", messages("error.supervision.start.invalid.date.multiple"), Seq(fields._1, fields._2))
          )
        }
      }
    }
