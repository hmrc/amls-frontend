...
trait RepeatingSection {
  ...
  def getData[T](cache: CacheMap)(implicit  formats: Format[T], key: MongoKey[T]): Seq[T] =
  ...
  def addData[T](credId:String, data: T)(implicit formats: Format[T], key: MongoKey[T], ec: ExecutionContext): Future[Int] = {
  ...
  def fetchAllAndUpdateStrict[T](credId: String, index: Int)(fn: (CacheMap, T) => T)
                                (implicit formats: Format[T], key: MongoKey[T], ec: ExecutionContext): Future[Option[CacheMap]] = {
