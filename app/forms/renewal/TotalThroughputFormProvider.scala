...
class TotalThroughputFormProvider @Inject() () extends Mappings {
  ...
  def apply(): Form[TotalThroughput] = Form(
    "throughput" -> enumerable[ExpectedThroughput](error)
      .transform[TotalThroughput](ExpectedThroughput.convert, TotalThroughput.convert)
  )
}
