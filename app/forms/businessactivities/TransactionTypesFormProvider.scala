...
class TransactionTypesFormProvider @Inject()() extends Mappings {
  ...
  def apply(): Form[TransactionTypes] = Form[TransactionTypes](
    mapping(
      "types" -> seq(enumerable[TransactionType](
        "error.required.ba.atleast.one.transaction.record")(TransactionTypes.enumerable))
        .verifying(
...
// TODO probably a better way to handle this, come back and see
  private def apply(transactionTypes: Seq[TransactionType], maybeName: Option[String]): TransactionTypes = (transactionTypes, maybeName) match {
    ...
    case (tt, Some(n)) if !tt.contains(DigitalOther) => throw new IllegalArgumentException("Cannot have name without digital software")
    case (tt, None) if tt.contains(DigitalOther) => throw new IllegalArgumentException("Cannot have digital software without name")
