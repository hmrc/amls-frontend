...
class StatusController @Inject() (
  ...
  view: YourRegistrationView
) extends AmlsBaseController(ds, cc) {

  def get(fromDuplicateSubmission: Boolean = false): Action[AnyContent] = authAction.async { implicit request =>
  ...
  def getFeeResponse(mlrRegNumber: Option[String], submissionStatus: SubmissionStatus, accountTypeId: (String, String))(
    ...
    (mlrRegNumber, submissionStatus) match {
      case (Some(mlNumber), (SubmissionReadyForReview | SubmissionDecisionApproved)) =>
        feeResponseService.getFeeResponse(mlNumber, accountTypeId)
      case _                                                                         => Future.successful(None)
    }

  def newSubmission: Action[AnyContent] = authAction.async { implicit request =>
    val redirect = for {
  ...
  private def getPageBasedOnStatus(
    ...
    statusInfo match {
      ...
      case (SubmissionDecisionApproved, _) | (SubmissionDecisionRejected, _) | (SubmissionDecisionRevoked, _) |
          (SubmissionDecisionExpired, _) | (SubmissionWithdrawn, _) | (DeRegistered, _) =>
        Future.successful(
          getDecisionPage(
            mlrRegNumber,
            statusInfo,
  ...
  private def getInitialSubmissionPage(
    mlrRegNumber: Option[String],
        ...
        Future.successful(
          Ok(
            view(
              regNo = mlrRegNumber.getOrElse(""),
              businessName = businessNameOption,
              yourRegistrationInfo = Some(
                applicationSubmissionReady(controllers.routes.RegistrationProgressController.get(), businessNameOption)
              ),
              unreadNotifications = unreadNotifications,
              registrationStatus = registrationStatus(
                amlsRegNo = mlrRegNumber,
      ...
      case (SubmissionDecisionRejected, _) =>
        Ok {
          view(
            ...
            unreadNotifications = unreadNotifications,
            registrationStatus = registrationStatus(status = statusInfo._1),
            feeInformation = None
          )
        }
      ...
      case (DeRegistered, _)               =>
        ...
        Ok {
          view(
            ...
            displayCheckOrUpdateLink = false,
            unreadNotifications = unreadNotifications,
            registrationStatus = registrationStatus(status = statusInfo._1, endDate = deregistrationDate),
            feeInformation = None
          )
        }
  ...
  private def getRenewalFlowPage(
    mlrRegNumber: Option[String],
    statusInfo: (SubmissionStatus, Option[ReadStatusResponse]),
    businessNameOption: Option[String],
    responsiblePeople: Option[Seq[ResponsiblePerson]],
        ...
        Future.successful(
          Ok(
            view(
              ...
              unreadNotifications = unreadNotifications,
              registrationStatus =
                registrationStatus(amlsRegNo = mlrRegNumber, status = statusInfo._1, endDate = renewalDate),
              feeInformation = Some(feeInformation(statusInfo._1))
            )
          )
        )
  ...
  private def getBusinessName(credId: String, safeId: Option[String], accountTypeId: (String, String))(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext
  ) =
  ...
  def countUnreadNotifications(amlsRefNo: Option[String], safeId: Option[String], accountTypeId: (String, String))(
    implicit headerCarrier: HeaderCarrier
  ): Future[Int] = {
