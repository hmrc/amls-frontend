...
class DeclarationController @Inject () (val dataCacheConnector: DataCacheConnector,
                                        ...
                                        view: DeclareView) extends AmlsBaseController(ds, cc) with Logging{

  def get(): Action[AnyContent] = authAction.async { implicit request =>

    //TODO: update state should happen on Post method, not on the Get, which should be readonly and idempotent
    lazy val whenSectionsComplete: Future[Result] = dataCacheConnector.fetch[AddPerson](request.credId, AddPerson.key) flatMap {
      case Some(addPerson) => {
