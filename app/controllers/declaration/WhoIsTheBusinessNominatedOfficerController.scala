...
class WhoIsTheBusinessNominatedOfficerController @Inject() (
  ...
  view: SelectBusinessNominatedOfficerView
) extends AmlsBaseController(ds, cc)
    with Logging {

  private def businessNominatedOfficerView(
    ...
    statusService.getStatus(amlsRegistrationNo, accountTypeId, cacheId) map {
      ...
      case SubmissionReadyForReview | SubmissionDecisionApproved =>
        status(view("submit.amendment.application", form, rp))
  ...
  def get: Action[AnyContent] = authAction.async { implicit request =>
    lazy val whenSectionsComplete = dataCacheConnector.fetchAll(request.credId) flatMap { optionalCache =>
      ...
      } yield businessNominatedOfficerView(
      ...
      )) getOrElse businessNominatedOfficerView(
        request.amlsRefNumber,
        request.accountTypeId,
        request.credId,
        Ok,
  ...
  private def updateNominatedOfficer(
    eventualMaybePeoples: Option[Seq[ResponsiblePerson]],
    data: BusinessNominatedOfficer
        ...
        val updatedList = ResponsiblePerson.filter(rpSeq).map { responsiblePerson =>
          if (responsiblePerson.personName.exists(name => name.fullNameWithoutSpace.equals(data.value))) {
            val position = responsiblePerson.positions.fold[Option[Positions]](None)(p =>
              Some(Positions(p.positions.+(NominatedOfficer), p.startDate))
            )
  ...
  private def validateRequest(
    ...
    cacheId: String,
    form: Form[BusinessNominatedOfficer]
  )(fn: BusinessNominatedOfficer => Future[Result])(implicit request: Request[AnyContent]): Future[Result] =
    form
      .bindFromRequest()
      .fold(
        formWithErrors =>
          dataCacheConnector.fetch[Seq[ResponsiblePerson]](cacheId, ResponsiblePerson.key) flatMap {
            case Some(data) =>
              businessNominatedOfficerView(
                amlsRegistrationNo,
                accountTypeId,
                cacheId,
                BadRequest,
