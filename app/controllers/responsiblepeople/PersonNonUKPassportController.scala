...
class PersonNonUKPassportController @Inject() (
      ...
      getData[ResponsiblePerson](request.credId, index) map { responsiblePerson =>
        responsiblePerson.fold(NotFound(notFoundView)) { person =>
          (person.personName, person.nonUKPassport) match {
            case (Some(name), Some(nonUKPassport)) =>
              Ok(view(formProvider().fill(nonUKPassport), edit, index, flow, name.titleName))
  ...
  private def redirectToNextPage(result: Option[Cache], index: Int, edit: Boolean, flow: Option[String])(implicit
