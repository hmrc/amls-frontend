...
class NotificationController @Inject()(val authEnrolmentsService: AuthEnrolmentsService,
        ...
        statusService.getReadStatus(amlsRegNo, request.accountTypeId) flatMap {
          case readStatus if readStatus.safeId.isDefined =>
            (for {
              ...
              Ok(notificationTemplateGenerator.contactTypeToView(contactType, (amlsRegNo, safeId), businessName, details, status, templateVersion))
  ...
  private def generateNotificationView(credId: String, safeId: String, refNumber: Option[String], accountTypeId: (String, String))
    ...
    (for {
      ...
      Ok(view(
        businessName,
        notificationTemplateGenerator.toTable(currentRecordsWithIndexes, "current-application-notifications", isPrevRegistration = false),
        previousRecordsWithIndexes.flatMap(recordsWithIndex =>
          if(recordsWithIndex.nonEmpty) {
            Some(notificationTemplateGenerator.toTable(recordsWithIndex, "previous-application-notifications", isPrevRegistration = true))
          } else {
            None
          }
        )
