  ...
  def get(edit: Boolean = false): Action[AnyContent] = authAction.async { implicit request =>
    dataCacheConnector.fetchAll(request.credId).map { optionalCache =>
      (for {
        ...
        view(
          formProvider().fill(involvedInOther.toBool),
          edit,
          businessMatching.prefixedAlphabeticalBusinessTypes(false)
        )
      )) getOrElse Ok(
        view(formProvider(), edit, businessMatching.prefixedAlphabeticalBusinessTypes(false))
        ...
        formWithErrors =>
          renewalService.getBusinessMatching(request.credId).map {
            case Some(businessMatching) =>
              BadRequest(view(formWithErrors, edit, businessMatching.prefixedAlphabeticalBusinessTypes(false)))
            case None                   => BadRequest(view(formWithErrors, edit, None))
          },
        involvedInOther =>
          renewalService
            .createOrUpdateRenewal(request.credId, r => updateRenewal(r, involvedInOther), initRenewal(involvedInOther))
            .map { _ =>
              if (involvedInOther) {
