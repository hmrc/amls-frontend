...
@Singleton
class BankAccountIbanController @Inject() (
  ...
  formProvider: BankAccountIBANNumberFormProvider,
  view: BankAccountIBANNumberView,
  ...
  def get(index: Int, edit: Boolean = false): Action[AnyContent] = authAction.async {
    implicit request: AuthorisedRequest[AnyContent] =>
      for {
        ...
        case Some(x @ BankDetails(_, _, Some(BankAccount(_, _, Some(data @ NonUKIBANNumber(_)))), _, _, _, _))
            if x.canEdit(status) =>
