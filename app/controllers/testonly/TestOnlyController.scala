...
class TestOnlyController @Inject() (implicit
  ...
  def showMongoCache: Action[AnyContent] = authAction.async { implicit request =>
    dataCacheConnector.fetchAll(request.credId).map { r =>
      r.fold(Ok(testOnlyViews.simpleView(s"No data in cache for [credId=${request.credId}]"))) { (cache: Cache) =>
  ...
  def updateMongo(fileName: String): Action[AnyContent] = authAction.async { implicit request =>
  ...
  def getPayment(ref: String) = authAction.async { implicit request =>
  ...
  def populateTP = authAction.async { implicit request =>
    val c = (1 until 1625) map { i =>
      TradingPremises(
        ...
        Some(
          YourTradingPremises(
            s"Test $i",
            Address(s"Trading Premises $i", None, None, None, "RE1 1ER"),
            Some(true),
            Some(LocalDate.now())
          )
        ),
    ...
    dataCacheConnector.save(request.credId, TradingPremises.key, c) map { _ =>
