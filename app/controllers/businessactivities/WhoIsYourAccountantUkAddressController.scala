...
class WhoIsYourAccountantUkAddressController @Inject()(val dataCacheConnector: DataCacheConnector,
                                                       ...
                                                       who_is_your_accountant_uk_address: who_is_your_accountant_uk_address) extends AmlsBaseController(ds, cc) {
    ...
    implicit request =>
      dataCacheConnector.fetch[BusinessActivities](request.credId, BusinessActivities.key) map {
        response =>
          ...
          Ok(who_is_your_accountant_uk_address(form, edit, ControllerHelper.accountantName(response)))
      }
  }
  ...
  def post(edit : Boolean = false) = authAction.async {
    implicit request =>
      Form2[UkAccountantsAddress](request.body) match {
        case f: InvalidForm =>
          dataCacheConnector.fetch[BusinessActivities](request.credId, BusinessActivities.key) map {
            response => BadRequest(who_is_your_accountant_uk_address(f, edit, ControllerHelper.accountantName(response)))
          }
