...
class DeregistrationCheckYourAnswersController @Inject() (
  ...
  view: DeregistrationCheckYourAnswersView
  ...
  private def fetchDeregistrationReason()(implicit request: AuthorisedRequest[AnyContent]) =
    EitherT.fromOptionF[Future, Result, DeregistrationReason](
      dataCacheConnector.fetch[DeregistrationReason](request.credId, DeregistrationReason.key),
      redirectToLandingPage
    )

  private def deregistrationReasonOthers(deregistrationReason: DeregistrationReason): Option[String] =
    deregistrationReason match {
