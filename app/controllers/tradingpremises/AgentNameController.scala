        ...
        case ValidForm(_, data) => {
          for {
            result <- updateDataStrict[TradingPremises](index) {
              case Some(tp) =>
                Some(TradingPremises(tp.registeringAgentPremises,tp.yourTradingPremises, tp.yourAgent,Some(data),tp.agentCompanyName, tp.whatDoesYourBusinessDoAtThisAddress, tp.msbServices))
            }
          } yield edit match {
