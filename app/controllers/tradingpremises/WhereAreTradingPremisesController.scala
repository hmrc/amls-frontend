  ...
  def get(index: Int, edit: Boolean = false) = Authorised.async {
    implicit authContext => implicit request =>
      getData[TradingPremises](index) map {
        case Some(TradingPremises(_,Some(data), _,_,_,_,_,_)) =>
        ...
        case ValidForm(_, ytp) => {
          for {
            _ <- updateDataStrict[TradingPremises](index) {
              ...
              case Some(tp) =>
                Some(TradingPremises(tp.registeringAgentPremises, Some(ytp), tp.businessStructure,tp.agentName,tp.agentCompanyName, tp.agentPartnership,tp.whatDoesYourBusinessDoAtThisAddress, tp.msbServices))
            }
