  ...
  def get(index: Int, edit: Boolean = false) = Authorised.async {
    implicit authContext => implicit request =>
      getData[TradingPremises](index) map {
        case Some(TradingPremises(_,Some(data), _,_,_, _,_)) =>
          ...
          for {
            _ <- updateDataStrict[TradingPremises](index) {
              ...
              case Some(tp) if ytp.isOwner =>
                Some(TradingPremises(None, Some(ytp), None, None,None,tp.whatDoesYourBusinessDoAtThisAddress, tp.msbServices))
              case Some(tp) =>
                Some(TradingPremises(tp.registeringAgentPremises, Some(ytp), tp.yourAgent,tp.agentName,tp.agentCompanyName, tp.whatDoesYourBusinessDoAtThisAddress, tp.msbServices))
            }
