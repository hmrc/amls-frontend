  ...
  def get(index: Int, edit: Boolean = false) = Authorised.async {
    implicit authContext => implicit request =>
      getData[TradingPremises](index) map {
        case Some(TradingPremises(Some(data), _,_, _,_)) =>
          ...
          for {
            _ <- updateDataStrict[TradingPremises](index) {
              ...
              case Some(tp) if ytp.isOwner =>
                Some(TradingPremises(Some(ytp), None ,None, tp.whatDoesYourBusinessDoAtThisAddress, tp.msbServices))
              case Some(tp) =>
                Some(TradingPremises(Some(ytp), tp.yourAgent,tp.agentCompanyName, tp.whatDoesYourBusinessDoAtThisAddress, tp.msbServices))
            }
