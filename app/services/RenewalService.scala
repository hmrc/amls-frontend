...
class RenewalService @Inject()(dataCache: DataCacheConnector) {
  ...
  def getSection(cache: CacheMap)(implicit authContext: AuthContext, headerCarrier: HeaderCarrier, ec: ExecutionContext) = {

    val notStarted = Section("renewal", NotStarted, hasChanged = false, controllers.renewal.routes.WhatYouNeedController.get())

    this.getRenewal(cache) map {
      case model if model.isComplete =>
        Section("renewal", Completed, model.hasChanged, controllers.renewal.routes.SummaryController.get())
      case Renewal(None, None, None, None, _, _, _, _, _, _, _, _, _) =>
        notStarted
      case model =>
        Section("renewal", Started, model.hasChanged, controllers.renewal.routes.WhatYouNeedController.get())
      case _ => notStarted
  ...
  def getRenewal(cache: CacheMap)(implicit authContext: AuthContext, headerCarrier: HeaderCarrier, ec: ExecutionContext) =
