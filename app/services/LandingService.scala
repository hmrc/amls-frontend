...
class LandingService @Inject() (
  ...
  def setAltCorrespondenceAddress(
    amlsRefNumber: String,
    ...
    lazy val etmpModel = OptionT.liftF(amlsConnector.view(amlsRefNumber, accountTypeId) map { v =>
      v.businessDetailsSection
    })

    (for {
      businessDetails <- cachedModel orElse etmpModel
      cacheMap        <-
        OptionT.liftF(cacheConnector.save[BusinessDetails](credId, BusinessDetails.key, fixAddress(businessDetails)))
  ...
  def refreshCache(amlsRefNumber: String, credId: String, accountTypeId: (String, String))(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext,
    messages: Messages
  ): Future[Cache] =
    for {
      ...
      subscriptionResponse   <-
        cacheConnector.fetch[SubscriptionResponse](credId, SubscriptionResponse.key).recover { case _ => None }
      amendVariationResponse <-
        cacheConnector.fetch[AmendVariationRenewalResponse](credId, AmendVariationRenewalResponse.key) recover {
          case _ => None
        }
  ...
  def writeEmptyBankDetails(bankDetailsSeq: Seq[BankDetails]): Seq[BankDetails] = {
    ...
    bankDetailsSeq match {
      case `empty` =>
        Seq(BankDetails(None, None, None, hasChanged = false, refreshedFromServer = true, None, hasAccepted = true))
      case _       =>
        bankDetailsSeq map { bank =>
   ...
   * Privates *
   ************/
  private def upsertCacheEntries(
    appCache: Cache,
    viewResponse: ViewResponse,
    subscriptionResponse: Option[SubscriptionResponse],
    amendVariationResponse: Option[AmendVariationRenewalResponse],
    amlsRegistrationNo: String,
    accountTypeId: (String, String),
    cacheId: String
  )(implicit hc: HeaderCarrier, ec: ExecutionContext, messages: Messages): Future[Cache] = {

    val cachedViewResponse =
      cacheConnector.upsertNewAuth[Option[ViewResponse]](appCache, ViewResponse.key, Some(viewResponse))

    val cachedBusinessMatching = cacheConnector
      .upsertNewAuth[BusinessMatching](cachedViewResponse, BusinessMatching.key, viewResponseSection(viewResponse))

    val cachedEstateAgentBusiness =
      cacheConnector.upsertNewAuth[Option[Eab]](cachedBusinessMatching, Eab.key, eabSection(viewResponse))

    val cachedTradingPremises = cacheConnector.upsertNewAuth[Option[Seq[TradingPremises]]](
      cachedEstateAgentBusiness,
      TradingPremises.key,
      tradingPremisesSection(viewResponse.tradingPremisesSection)
    )

    val cachedBusinessDetails = cacheConnector
      .upsertNewAuth[BusinessDetails](cachedTradingPremises, BusinessDetails.key, aboutSection(viewResponse))

    val cachedBankDetails = cacheConnector.upsertNewAuth[Seq[BankDetails]](
      cachedBusinessDetails,
      BankDetails.key,
      writeEmptyBankDetails(viewResponse.bankDetailsSection)
    )

    val cachedAddPerson =
      cacheConnector.upsertNewAuth[AddPerson](cachedBankDetails, AddPerson.key, viewResponse.aboutYouSection)

    val cachedBusinessActivities = cacheConnector
      .upsertNewAuth[BusinessActivities](cachedAddPerson, BusinessActivities.key, activitySection(viewResponse))

    val cachedTcsp =
      cacheConnector.upsertNewAuth[Option[Tcsp]](cachedBusinessActivities, Tcsp.key, tcspSection(viewResponse))

    val cachedAsp = cacheConnector.upsertNewAuth[Option[Asp]](cachedTcsp, Asp.key, aspSection(viewResponse))

    val cachedMoneyServiceBusiness = cacheConnector
      .upsertNewAuth[Option[MoneyServiceBusiness]](cachedAsp, MoneyServiceBusiness.key, msbSection(viewResponse))

    val cachedHvd =
      cacheConnector.upsertNewAuth[Option[Hvd]](cachedMoneyServiceBusiness, Hvd.key, hvdSection(viewResponse))

    val cachedAmp = cacheConnector.upsertNewAuth[Option[Amp]](cachedHvd, Amp.key, ampSection(viewResponse))

    val cachedSupervision =
      cacheConnector.upsertNewAuth[Option[Supervision]](cachedAmp, Supervision.key, supervisionSection(viewResponse))

    val cachedSubscriptionResponse = cacheConnector
      .upsertNewAuth[Option[SubscriptionResponse]](cachedSupervision, SubscriptionResponse.key, subscriptionResponse)

    val cachedAmendVariationRenewalResponse = cacheConnector.upsertNewAuth[Option[AmendVariationRenewalResponse]](
      cachedSubscriptionResponse,
      AmendVariationRenewalResponse.key,
      amendVariationResponse
    )

    val cachedResponsiblePerson = cacheConnector.upsertNewAuth[Option[Seq[ResponsiblePerson]]](
      cachedAmendVariationRenewalResponse,
      ResponsiblePerson.key,
      responsiblePeopleSection(viewResponse.responsiblePeopleSection)
    )

    val cachedRenewal =
      saveRenewalData(viewResponse, cachedResponsiblePerson, amlsRegistrationNo, accountTypeId, cacheId)
        ...
        val renewal = Some(
          Renewal(
            involvedInOtherActivities =
              viewResponse.businessActivitiesSection.involvedInOther.map(i => BAInvolvedInOther.convert(i)),
            businessTurnover = viewResponse.businessActivitiesSection.expectedBusinessTurnover.map(data =>
              ExpectedBusinessTurnover.convert(data)
            ),
            turnover = viewResponse.businessActivitiesSection.expectedAMLSTurnover.map(data =>
              ExpectedAMLSTurnover.convert(data)
            ),
            ampTurnover = viewResponse.ampSection map { amp => Amp.convert(amp) },
            customersOutsideUK =
              viewResponse.businessActivitiesSection.customersOutsideUK.map(c => CustomersOutsideUK(c.countries)),
            percentageOfCashPaymentOver15000 = viewResponse.hvdSection.fold[Option[PercentageOfCashPaymentOver15000]](
              None
            ...
            totalThroughput = viewResponse.msbSection
              .fold[Option[TotalThroughput]](None)(_.throughput.map(t => ExpectedThroughput.convert(t))),
            whichCurrencies = viewResponse.msbSection
              .fold[Option[WhichCurrencies]](None)(_.whichCurrencies.map(c => MsbWhichCurrencies.convert(c))),
            ...
            mostTransactions = viewResponse.msbSection
              .fold[Option[MostTransactions]](None)(_.mostTransactions.map(m => MostTransactions(m.countries))),
  ...
  private def fixAddress(model: BusinessDetails): BusinessDetails =
    (model.correspondenceAddress, model.altCorrespondenceAddress) match {
