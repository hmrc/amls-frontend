...
class RecoverActivitiesService @Inject()(amlsConnector: AmlsConnector,
                                         dataCacheConnector: DataCacheConnector) extends Logging {

  def recover(request: AuthorisedRequest[_])(implicit messages: Messages,
                                             hc: HeaderCarrier,
                                             ec: ExecutionContext): Future[Boolean] = request.amlsRefNumber match {
    case Some(refNumber) =>
      for {
        ...
        _ <- dataCacheConnector.save[BusinessMatching](request.credId, BusinessMatching.key, refreshedBusinessMatching)
      } yield {
        ...
        listOfBusinessTypes match {
          case Some(list) if list.nonEmpty =>
            true
          case Some(_) =>
            logger.warn("[RecoverActivitiesService][recover] - Empty list of business types returned by DES, unable to fix record")
            false
          case _ =>
            logger.warn("[RecoverActivitiesService][recover] - No BusinessActivities section returned by DES, unable to fix record")
            false
        }
      }
    case None =>
