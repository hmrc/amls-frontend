...
class MongoCacheClient @Inject() (
  ...
  /** Inserts data into the existing cache object in memory given the specified key. If the data does not exist, it will
    * be created.
    */
  ...
  /** Fetches everything from the database & stores it in in-memory cache
    ...
    *   of user
    * @return
    *   cache containing all data saved against the user
    */
