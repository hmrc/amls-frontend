...
object ActivityStartDate {
  ...
  implicit val formRule: Rule[UrlFormEncoded, ActivityStartDate] = From[UrlFormEncoded] { __ =>
    import jto.validation.forms.Rules._
      (__ \ "startDate").read(newAllowedPastAndFutureDateRule("error.required.tp.address.date", "error.invalid.date.after.1700", "error.required.tp.agent.date.past")) map ActivityStartDate.apply
  }
