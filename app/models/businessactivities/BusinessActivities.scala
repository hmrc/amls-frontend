...
case class BusinessActivities(
  ...
  def businessFranchise(p: BusinessFranchise): BusinessActivities =
    this.copy(
      businessFranchise = Some(p),
      hasChanged = hasChanged || !this.businessFranchise.contains(p),
  ...
  def expectedAMLSTurnover(p: ExpectedAMLSTurnover): BusinessActivities =
    this.copy(
      expectedAMLSTurnover = Some(p),
      hasChanged = hasChanged || !this.expectedAMLSTurnover.contains(p),
  ...
  def expectedBusinessTurnover(p: ExpectedBusinessTurnover): BusinessActivities =
    this.copy(
      expectedBusinessTurnover = Some(p),
      hasChanged = hasChanged || !this.expectedBusinessTurnover.contains(p),
      hasAccepted = hasAccepted && this.expectedBusinessTurnover.contains(p)
    )

  def involvedInOther(p: InvolvedInOther): BusinessActivities =
    this.copy(
      involvedInOther = Some(p),
      hasChanged = hasChanged || !this.involvedInOther.contains(p),
  ...
  def identifySuspiciousActivity(p: IdentifySuspiciousActivity): BusinessActivities =
    this.copy(
      identifySuspiciousActivity = Some(p),
      hasChanged = hasChanged || !this.identifySuspiciousActivity.contains(p),
  ...
  def transactionRecord(isRecorded: Boolean): BusinessActivities = {
    ...
    this.copy(
      transactionRecord = Some(isRecorded),
      transactionRecordTypes = types,
  ...
  def transactionRecordTypes(types: TransactionTypes): BusinessActivities =
    this.copy(
      transactionRecordTypes = Some(types),
      hasChanged = hasChanged || !this.transactionRecordTypes.contains(types),
  ...
  def customersOutsideUK(p: CustomersOutsideUK): BusinessActivities =
    this.copy(
      customersOutsideUK = Some(p),
      hasChanged = hasChanged || !this.customersOutsideUK.contains(p),
      hasAccepted = hasAccepted && this.customersOutsideUK.contains(p)
    )

  def ncaRegistered(p: NCARegistered): BusinessActivities =
    this.copy(
      ncaRegistered = Some(p),
      hasChanged = hasChanged || !this.ncaRegistered.contains(p),
  ...
  def accountantForAMLSRegulations(p: Option[AccountantForAMLSRegulations]): BusinessActivities =
    this.copy(
      accountantForAMLSRegulations = p,
      hasChanged = hasChanged || !this.accountantForAMLSRegulations.equals(p),
  ...
  def riskAssessmentPolicy(p: RiskAssessmentPolicy): BusinessActivities =
    this.copy(
      riskAssessmentPolicy = Some(p),
      hasChanged = hasChanged || !this.riskAssessmentPolicy.contains(p),
  ...
  def riskAssessmentHasPolicy(p: RiskAssessmentHasPolicy): BusinessActivities = {
    ...
    this.copy(
      riskAssessmentPolicy = Some(newRiskAssessmentPolicy),
      hasChanged = hasChanged || !this.riskAssessmentPolicy.contains(p),
      hasAccepted = hasAccepted && this.riskAssessmentPolicy.contains(p)
    )
  }
  ...
  def howManyEmployees(p: HowManyEmployees): BusinessActivities =
    this.copy(
      howManyEmployees = Some(p),
      hasChanged = hasChanged || !this.howManyEmployees.contains(p),
  ...
  def whoIsYourAccountant(p: Option[WhoIsYourAccountant]): BusinessActivities =
    this.copy(
      whoIsYourAccountant = p,
      hasChanged = hasChanged || !this.whoIsYourAccountant.equals(p),
  ...
  def taxMatters(p: Option[TaxMatters]): BusinessActivities =
    this.copy(
      taxMatters = p,
      hasChanged = hasChanged || !this.taxMatters.equals(p),
...
object BusinessActivities extends Logging {
  ...
  def section(implicit cache: Cache): Section = {
    ...
    val bmBusinessActivities =
      ControllerHelper.getBusinessActivity(cache.getEntry[BusinessMatching](BusinessMatching.key))
    cache.getEntry[BusinessActivities](key).fold(notStarted) { model =>
  ...
  implicit val writes: Writes[BusinessActivities] = Writes[BusinessActivities] { model =>
    ...
    Seq(
      model.involvedInOther.map(v => Json.toJson(v).as[JsObject]),
      model.expectedBusinessTurnover.map(v => Json.toJson(v)(ExpectedBusinessTurnover.jsonWrites).as[JsObject]),
      model.expectedAMLSTurnover.map(v => Json.toJson(v)(ExpectedAMLSTurnover.jsonWrites).as[JsObject]),
      model.businessFranchise.map(v => Json.toJson(v).as[JsObject]),
      ...
      model.customersOutsideUK.map(v => Json.toJson(v).as[JsObject]),
      model.ncaRegistered.map(v => Json.toJson(v).as[JsObject]),
      model.accountantForAMLSRegulations.map(v => Json.toJson(v).as[JsObject]),
      model.identifySuspiciousActivity.map(v => Json.toJson(v).as[JsObject]),
      model.riskAssessmentPolicy.map(v => Json.toJson(v).as[JsObject]),
      model.howManyEmployees.map(v => Json.toJson(v).as[JsObject]),
      model.whoIsYourAccountant.map(v => Json.toJson(v).as[JsObject]),
      model.taxMatters.map(v => Json.toJson(v).as[JsObject])
