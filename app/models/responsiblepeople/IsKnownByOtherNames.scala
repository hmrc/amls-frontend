...
object IsKnownByOtherNames {
  ...
  import play.api.data.mapping.forms.Rules._
/*
    (__ \ "isKnownByOtherNames").read[Boolean].map[IsKnownByOtherNames] {
      _ => Success(IsKnownByOtherNamesNo)
    }
*/
    (__ \ "isKnownByOtherNames").read[Boolean] flatMap {
      case true =>
        (__ \ "otherfirstnames").read(firstNameType) fmap (IsKnownByOtherNamesYes.apply)
