...
object CETransactionsInNext12Months {
  ...
  private val txnAmountType = notEmptyStrip andThen
    notEmpty.withMessage("error.required.msb.ce.transactions.in.12months") andThen
    maxLength(maxCharLength).withMessage("error.invalid.msb.ce.transactions.in.12months") andThen
    regexWithMsg("^[0-9]{1,11}".r, "error.invalid.msb.ce.transactions.in.12months.number")
