...
case class Asp(
  ...
  def services(p: ServicesOfBusiness): Asp =
    this.copy(services = Some(p), hasChanged = hasChanged || !this.services.contains(p), hasAccepted = this.services.contains(p))

  def otherBusinessTaxMatters(p: OtherBusinessTaxMatters): Asp =
    this.copy(otherBusinessTaxMatters = Some(p), hasChanged = hasChanged || !this.otherBusinessTaxMatters.contains(p), hasAccepted = this.otherBusinessTaxMatters.contains(p))
