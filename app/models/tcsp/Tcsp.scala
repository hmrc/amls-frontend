...
object Tcsp {
  ...
  def section(implicit cache: CacheMap): Section = {
    ...
    cache.getEntry[Tcsp](key).fold(notStarted) {
      model =>
        if (model.isComplete) {
          Section(messageKey, Completed, false, controllers.tcsp.routes.SummaryController.get())
        } else {
          Section(messageKey, Started, false, controllers.tcsp.routes.WhatYouNeedController.get())
        }
    }
  }
